...
module.exports = function(assets, options, callback) {
    ...
    Step(function() {
        ...
        for(var name in assets) {
            ...
            assetGroup.files = assetGroup.files || [];
            assetGroup.attribute = assetGroup.attribute || "";
        ...
        if(err) {
            console.log("Error while processing assets : "+err.message);
        } else {
    ...
    return function(req, res, next) {
        res.locals.asset = function(name, options) {
            ...
            if(assetGroup !== undefined) {
                var assetHtml = assetGroup.urls.map(function(url) {
                    return assetToHtml(options.prefix+url, assetGroup, options.attribute);
                }).join("\n");
                return '<!-- start asset "'+name+'" -->\n'+assetHtml+'<!-- end asset -->\n';
            } else {
...
function startupHook(options) {
    var hooks = {
        development: function(assetGroup, callback) {
            if(assetGroup.requirejs) {
                switch(assetGroup.type) {
                    case "js" :
                        assetGroup.files.push(assetGroup.requirejs.lib);
                        assetGroup.files.push(assetGroup.requirejs.mainConfigFile);
                        assetGroup.attribute += ' data-main="'+path.join(assetGroup.route, assetGroup.requirejs.main)+'"';
                        break;
                    default :
                        assetGroup.files.push(assetGroup.requirejs.main);
                }
            }
            callback && callback();
            // do nothing
        ...
        production: function(assetGroup, callback) {
            ...
            if(options.process) {
                if(assetGroup.requirejs) {
                    var config = createRequireJSOptimizerConfig(assetGroup, options);
...
function createRequireJSOptimizerConfig(assetGroup, options) {
    var dstFile = path.join(options.buildDir, assetGroup.name);
    switch(assetGroup.type) {
        case "js":
            return {
                baseUrl: assetGroup.dir,
                name: assetGroup.requirejs.main,
                mainConfigFile: path.join(assetGroup.dir, assetGroup.requirejs.mainConfigFile),
                out: dstFile
            };
        case "css":
            return {
                cssIn: path.join(assetGroup.dir, assetGroup.requirejs.main),
                out: dstFile,
