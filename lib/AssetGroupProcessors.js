    ...
    var hooks = {
        development: {
            createAsset: function(assetGroup, fileInfo) {
                fileInfo.filepath = path.join(assetGroup.config.dir, fileInfo.name || fileInfo);
                return assetGroup.createDefaultAsset(fileInfo);
            },
            default: function(assetGroup, callback) {
                var self = this;
                assetGroup.assets = assetGroup.config.files.map(function(fileInfo) {
                    return self.createAsset(assetGroup, fileInfo);
                });
                callback && callback();
            },
            requirejs: function(assetGroup, callback) {
                ...
                assetGroup.assets = [
                    this.createAsset(assetGroup, {
                        ...
                        attributes  : [{
                            "data-main": this.createAsset(assetGroup, assetGroup.config.main)
                        }]
                    }),
            ...
            less: function(assetGroup, callback) {
                ...
                assetGroup.assets = [
                    this.createAsset(assetGroup, { name : assetGroup.config.main, type : "less" }),
                ...
                if(assetGroup.config.lib && assetGroup.config.includeLib === true) {
                    ...
                    assetGroup.assets = [
                        this.createAsset(assetGroup, {
                            name        : assetGroup.config.lib,
                            attributes  : [{
                                "data-main": assetGroup.config.main
                            }]
                        })
                    ];
                }
            ...
            js: function(assetGroup, callback) {
                new compressor.minify({
                    ...
                    fileIn      : assetGroup.config.files.map(function(file) {
                        return path.join(assetGroup.config.dir, file);
                    }),
